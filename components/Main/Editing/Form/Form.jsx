...
const Form = (props) => {
    const [title, setTitle] = useState(props.good.title )
    const [description, setDescription] = useState(props.good.description || '')
    const [date, setDate] = useState(props.date.date || '')
    const [time, setTime] = useState(props.date.time || '')
    const [discont, setDiscont] = useState(props.good.discont || '')
    const [cost, setCost] = useState(props.good.cost)
    const [image, setImage] = useState(props.good.image_url)
    ...
    const checkImage = () => {
        if(image)
            ...
            setError('Нет ссылки на картинку!')
            return false}
    }
